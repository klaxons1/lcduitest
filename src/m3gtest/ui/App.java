/*
 * M3G Tester - UI App shell, same as LCDUI tester but for M3G.
 */
package m3gtest.ui;
...
public class App implements CommandListener {
    ...
    private final List mainMenu = new List(\"M3G Test Suite\", List.IMPLICIT);
    private final List listScreen = new List(\"\", List.IMPLICIT);
    private int listKind = LIST_CATEGORY;
    ...
    private TestSuite[] categorySuites;
    ...
    public void start() {
        ...
        for (int i = 0; i < Suites.categoryCount(); i++) {
            mainMenu.append(Suites.categoryName(i) + \" (\" + Suites.categorySuiteCount(i) + \")\", null);
        }
        mainMenu.append(\"Visual demos (\" + Demos.all().length + \")\", null);
        ...
        if (Env.flag(\"m3gtest.autorun\") || Env.flag(\"lcduitest.autorun\")) {
    ...
    private void showCategory(int category) {
        categorySuites = Suites.byCategory(category);
        categoryTitle = Suites.categoryName(category);
        listKind = LIST_CATEGORY;
        listScreen.setTitle(categoryTitle);
        listScreen.deleteAll();
        listScreen.removeCommand(runAllCommand);
        listScreen.removeCommand(saveCommand);
        for (int i = 0; i < categorySuites.length; i++) {
            TestSuite suite = categorySuites[i];
    ...
    private void showDemos() {
        ...
        for (int i = 0; i < demos.length; i++) {
            listScreen.append(demos[i].getTitle(), null);
        }
        listScreen.addCommand(backCommand);
        listScreen.addCommand(okCommand);
        listScreen.setCommandListener(this);
    ...
    private void showHelp() {
        ...
        sb.append(\"Every menu entry runs a group of test cases that exercises the javax.microedition.m3g API of this device or emulator against the JSR-184 specification.\\n\\n\");
        sb.append(\"Marks in the result list:\\n\");
        sb.append(\"  +  pass\\n\");
        sb.append(\"  x  failure (specification violated)\\n\");
        sb.append(\"  !  error (unexpected exception)\\n\");
        sb.append(\"  i  observation (implementation defined, never a failure)\\n\");
        sb.append(\"  M  manual test (needs a human)\\n\\n\");
        sb.append(\"Automated (CI) mode: start the MIDlet with\\n\");
        sb.append(\"  m3gtest.autorun=true\\n\");
        sb.append(\"(JAD attribute M3gtest-Autorun: true, or -Dm3gtest.autorun=true) to run everything, print a machine readable log on stdout and store the summary in the RecordStore. Add m3gtest.exit=true to shut the MIDlet down when the run is over.\\n\");
        sb.append(\"\\nThis tester is modelled after LCDUITester, but covers M3G classes:\\n\");
        sb.append(\"Object3D, Transform, Graphics3D, Loader, VertexArray, VertexBuffer, IndexBuffer, Image2D, Texture2D, Appearance, Material, PolygonMode, CompositingMode, Fog, Background, Node, Group, World, Camera, Light, Mesh, MorphingMesh, SkinnedMesh, Sprite3D, AnimationController, AnimationTrack, KeyframeSequence, RayIntersection.\\n\");
    ...
    private void showLastStoredRun() {
        ...
        if (sb.length() == 0) {
            sb.append(\"no stored run\\n\\n(run a test group with m3gtest.store=true or use 'Save report' on the result screen)\");
        }
    ...
    private class Session implements Runnable {
        ...
        public void run() {
            ...
            results.finish();
            results.announceEnd();
            if (Env.flag(\"m3gtest.store\") || Env.flag(\"lcduitest.store\")) {
                results.save();
            }
            display.callSerially(new Runnable() {
                public void run() {
                    showRunResults();
                    if (auto && (Env.flag(\"m3gtest.exit\") || Env.flag(\"lcduitest.exit\"))) {
    ...
    private void showSuiteRunResults(int index) {
        ...
        listScreen.setTitle(selectedSuiteRun.suite.getId() + \": \" + selectedSuiteRun.stats.passed + \"/\" + selectedSuiteRun.stats.total);
        listScreen.deleteAll();
    ...
    private void showTestDetail(int index) {
        ...
        if (selectedResult.getSeverity() == TestCase.MANUAL || selectedResult.getSeverity() == TestCase.INFO) {
    ...
    private void fillDetail(TestResult result) {
        ...
        detailForm.append(new StringItem(\"status\", result.getStatusText() + \" (\" + Results.severityName(result.getSeverity()) + \", \" + result.getMillis() + \" ms)\"));
    ...
    public void commandAction(Command c, Displayable d) {
        ...
        if (c == saveCommand) {
            if (results != null) {
                results.save();
                Alert alert = new Alert(\"Stored\", \"The summary was written to the RecordStore \" + Results.STORE_NAME + \".\", null, AlertType.INFO);
    ...
    private void handleSelect(Displayable d) {
        if (d == mainMenu) {
            ...
            int categories = Suites.categoryCount();
            if (index == 0) {
                runAll(false);
                ...
                showDemos();
            } else if (index == categories + 2) {
                showEnvironment();
            } else if (index == categories + 3) {
                showLastStoredRun();
            } else {
                showHelp();
            }
        ...
        if (listKind == LIST_CATEGORY) {
            TestSuite suite = categorySuites[index];
            TestSuite[] one = new TestSuite[1];
            one[0] = suite;
            runSuites(one, suite.getId(), false);
        } else if (listKind == LIST_DEMOS) {
            Demo demo = Demos.all()[index];
            Displayable screen = demo.createScreen();
