 ...
 *
 * Application shell: menu tree, test execution, progress display, result
 * browser and the automated (CI) mode.
 *
 *   main menu
 *     + Run ALL
 *     + category  -> suite list -> run -> results -> per test detail
 *     + Visual demos -> a Canvas to look at
 *     + Environment  -> MIDP system properties of this device
 *     + Last stored run (RecordStore)
 *     + Help
 *
 * All screens share a single CommandListener and navigate with a one level
 * "previous screen" stack, which is the usual pattern for small MIDlets.
 */
package lcduitest.ui;
...
public class App implements CommandListener {
    ...
    private final List mainMenu = new List("LCDUI Test Suite", List.IMPLICIT);
    ...
    private void showHelp() {
        ...
        sb.append("LCDUI conformance test MIDlet for MIDP 2.0.\n\n");
        sb.append("Every menu entry runs a group of test cases that exercises the "
                + "javax.microedition.lcdui API of this device or emulator against the "
        ...
        sb.append("  lcduitest.autorun=true\n");
        sb.append("(JAD attribute Lcduitest-Autorun: true, or -Dlcduitest.autorun=true) "
                + "to run everything, print a machine readable log on stdout and store "
                + "the summary in the RecordStore. Add lcduitest.exit=true to shut the "
                + "MIDlet down when the run is over.\n");
    ...
    private void showLastStoredRun() {
        ...
        if (sb.length() == 0) {
            sb.append("no stored run\n\n(run a test group with lcduitest.store=true "
    ...
    private class Session implements Runnable {
        ...
        public void run() {
            ...
            results.finish();
            results.announceEnd();
            if (Env.flag("lcduitest.store")) {
                results.save();
            }
            display.callSerially(new Runnable() {
                public void run() {
                    showRunResults();
                    if (auto && Env.flag("lcduitest.exit")) {
    ...
    /** Re-runs one test case on a worker thread (used for observations/manual). */
    ...
    /** Runs a test case that is not part of the current result list. */
    private void runSingle(final TestCase test) {
        showText(test.getName(), "running...");
        Thread thread = new Thread(new Runnable() {
            public void run() {
                final TestResult result = TestRunner.execute(test);
                Results.announceTest("single", result);
                display.callSerially(new Runnable() {
                    public void run() {
                        TextBox box = new TextBox(test.getName(), describe(test, result), 2048, 0);
                        box.addCommand(backCommand);
                        box.setCommandListener(App.this);
                        go(box);
                    }
                });
            }
